...
export function registerInteractiveRoutes(app: Express): void {
  ...
  // Backward-compatible aliases used by existing UI modules
  app.post("/api/interactive/biology/analyze-dna", async (req, res) => {
    try {
      const sampleSequence = req.body?.sampleSequence ?? req.body?.sequence;
      const referenceSequence = req.body?.referenceSequence;
      if (!sampleSequence) {
        return res.status(400).json({ success: false, error: "Sample sequence required" });
      }
      const result = biologyModule.analyzeDNA(sampleSequence, referenceSequence);
  ...
  app.post("/api/interactive/biology/detect-pathogen", async (req, res) => {
    try {
      const result = await biologyModule.detectPathogen({
        type: req.body?.type ?? req.body?.sampleType,
  ...
  app.post("/api/interactive/medical/analyze", async (req, res) => {
    try {
      const { symptoms = [], vitals = {}, patientInfo } = req.body || {};
      const normalizedSymptoms = Array.isArray(symptoms) ? symptoms : [];
      const diagnosis = await medicalDiagnostics.analyzeSymptoms(normalizedSymptoms, patientInfo);
      const vitalsAnalysis = medicalDiagnostics.analyzeVitalSigns(vitals);
      res.json({
        success: true,
        diagnosis,
  ...
  app.post("/api/interactive/blood/start", async (req, res) => {
    try {
      const { patientId, armPosition } = req.body || {};
      const session = await bloodSamplingSystem.startSamplingSession(patientId, armPosition || "left");
      res.json({
