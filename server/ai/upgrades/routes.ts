...
export function registerAdvancedUpgradeRoutes(app: Express): void {
  ...
  app.post("/api/upgrades/quantum/create", async (req, res) => {
    try {
      const qubits = Number(req.body?.qubits ?? req.body?.numQubits ?? 2);
      const gates = Array.isArray(req.body?.gates) ? req.body.gates : [];
      const name = req.body?.name || `Circuit-${Date.now()}`;
      const circuit = quantumNeuralNetworks.createCircuit(name, qubits, gates);
      res.json({
        success: true,
        circuit: { id: circuit.id, name: circuit.name, qubits: circuit.qubits.length, gates: circuit.gates.length },
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message || "Failed to create circuit" });
    }
  ...
  app.post("/api/upgrades/quantum/simulate", async (req, res) => {
    try {
      const { circuitId, shots = 1024 } = req.body || {};
      if (!circuitId) {
        return res.status(400).json({ error: "Circuit ID is required" });
      ...
      res.json({
        ...
        result: {
          ...
          probabilities: result.probabilities,
          fidelity: result.coherenceScore,
          executionTime: result.executionTime,
        },
      });
    } catch (error: any) {
