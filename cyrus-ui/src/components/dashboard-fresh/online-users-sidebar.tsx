...
export function OnlineUsersSidebar({ className }: OnlineUsersSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "relative flex h-full min-h-[26rem] flex-col overflow-hidden rounded-2xl border border-slate-200 p-3.5 cyrus-xs-live-panel-sidebar",
        DASHBOARD_DARK_CONSOLE_BG,
        DASHBOARD_CONSOLE_SHADOW,
        className,
      )}
    ...
    >
      <div className="mb-2.5 flex shrink-0 items-center justify-between border-b border-slate-200 pb-2.5">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl border border-sky-200 bg-sky-50">
            <Users className="h-4 w-4 text-sky-600" aria-hidden />
          </div>
          <div>
            <p className="text-[10px] font-mono uppercase tracking-[0.28em] text-slate-500">Live panel</p>
            <h3 className="text-sm font-semibold text-slate-900">Activity</h3>
          </div>
        </div>
        <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-mono text-emerald-700">
      ...
      {isLoading ? (
        <p className="px-1 text-xs text-slate-500">Loading active operators…</p>
      ) : users.length === 0 ? (
        <p className="px-1 text-xs text-slate-500">No active operators yet.</p>
      ) : (
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
          {users.map((u) => (
            <div key={u.id} className={`px-3 py-2.5 text-slate-800 ${DASHBOARD_DARK_CONSOLE_INNER}`}>
              <div className="flex items-center gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-[9px] font-mono uppercase tracking-wide text-slate-500">
              ...
              {u.live ? (
                <Link href="/comms">
                  <button
                    type="button"
                    className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-700 transition hover:bg-sky-50 hover:text-sky-700"
