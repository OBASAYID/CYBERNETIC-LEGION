...
import { pgTable, text, varchar, timestamp, boolean, jsonb, integer, primaryKey } from "drizzle-orm/pg-core";
...
export const directMessages = pgTable("direct_messages", {
  ...
  /** e.g. image/jpeg, audio/mpeg (optional; can infer from file name) */
...
/** Public / shared feed in Comms (Pshare) — app-wide or audience-selected, optional comments. */
export const psharePosts = pgTable("pshare_posts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const pshareComments = pgTable("pshare_comments", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  postId: varchar("post_id").notNull().references(() => psharePosts.id, { onDelete: "cascade" }),
  authorId: varchar("author_id").notNull(),
  body: text("body").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const pshareLikes = pgTable(
  "pshare_likes",
  {
    postId: varchar("post_id")
      .notNull()
      .references(() => psharePosts.id, { onDelete: "cascade" }),
    userId: varchar("user_id").notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
